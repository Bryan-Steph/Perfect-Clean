        ...
        //  <section className='sm:flex block w-full'>
         <div className='sm:w-1/2 w-full sm:mt-10'>
                 ...
                 fixed sm:-top-15 -top-10 backdrop-blur-md bg-white/30 rounded-full shadow-md z-50 mt-20  h-20'>
                        ...
                        {!isMenuOpen ? 
                        <img id='menu-btn' onClick={() => toggleMenu()} className='w-5 h-8 sm:hidden ml-55 m-5 mt-5 ' src={Menu} alt="" />
                        :
                          <span onClick={() => toggleMenu()} className='w-5  sm:hidden ml-55 m-5 text-4xl  font-bold'>X</span>
                      }
                        
                        </div>
                        <div id='main-menu' className={
                            'flex flex-col md:flex-row gap-6 bg-blue-400 md:bg-transparent rounded-lg items-center  ' + `${isMenuOpen? ' flex' : 'md:flex hidden'}` 
                        }>
